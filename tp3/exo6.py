...
# math.pow(x, y) pour calculer 𝑥􀯬
    ...
    #     p3=Point(xp,yp)
    #     return p3
# Implémentez une version statique (méthode décorée par @staticmethod) des deux méthodes
# calculant la distance et le milieu.
    @staticmethod
    def calculer_distance( p1,p2):
     distance= math.sqrt(math.pow((p1.abs-p2.abs),2)+math.pow((p1.ord-p2.ord),2))
     return distance
    # Ecrivez la méthode calculer_milieu(self, p: ’Point’) -> ’Point’ qui permet de calculer
    @staticmethod
    def calculer_milieu(p1, p2):
        yp=(p1.ord+p2.ord)/2
        xp=(p1.abs+p2.abs)/2
        p3=Point(xp,yp)
        return p3
